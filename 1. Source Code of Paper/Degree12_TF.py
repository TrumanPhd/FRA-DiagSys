...
dataset = Disable_degreee_TF_Dataset()
...
for test_num in range(10):
    ...
    model = FiveClassClassifierWithSoftmax(dropout=False,num_classes=13).to(device)
    ...
    training_name = "Fault Degree Recognition "+str(test_num)  # 替换为您希望的名称
    log_dir = os.path.join(target_folder, training_name)

    # 创建SummaryWriter对象并指定保存路径
    writer = SummaryWriter(log_dir)

    # 初始化变量以跟踪最佳模型参数和验证损失
    best_model_params = None
    best_validation_loss = float('inf')
    early_stopping_patience = 60  # 设定早停的耐心值
    early_stopping_counter = 60  # 用于计算连续验证损失上升的次数
