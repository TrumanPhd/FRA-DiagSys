...
# 创建标签列
dataset['label'] = label_degree_EEandTF()

# 保存TabularDataset对象为CSV文件（AutoGluon要求数据保存为CSV）
dataset.to_csv('g2d_dataset.csv')
...
predictor = TabularPredictor(label=label, path='autogluon_models').fit(train_data, presets='best_quality')#, use_gpu=True)
