...
class LearnableMatrix(nn.Module):
    #this is a learnable matrix
    #input: (batch_size, seq_len) --> output: (batch_size, emb_dim, seq_len)
    def __init__(self, in_features=2000 , emb_dim=4):
        super().__init__()
        self.fc1 = nn.Linear(in_features=in_features,out_features=in_features)
        self.fc2 = nn.Linear(in_features=in_features,out_features=in_features)        
        self.fc3 = nn.Linear(in_features=in_features,out_features=in_features)        
        self.fc4 = nn.Linear(in_features=in_features,out_features=in_features)        
        self.emb_dim = emb_dim
            
    def forward(self,x):
        x = x.unsqueeze(1)
        return torch.cat((self.fc1(x),self.fc2(x),self.fc3(x),self.fc4(x)),dim=1)
"""
...
class LearnableMatrix(nn.Module):
    def __init__(self, in_features=2000, emb_dim=4):
        ...
        self.emb_dim = emb_dim
        self.fc_layers = nn.ModuleList([nn.Linear(in_features, in_features) for _ in range(emb_dim)])
