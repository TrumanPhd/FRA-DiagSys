...
class demo1(nn.Module):
    def __init__(self, input_size=2000, num_task=5):
        super(demo1, self).__init__()
        self.LM = LearnableMatrix()
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size, seq_len, features)  # 准备输入给LSTM
        output, (h_n, c_n) = self.NaiveBiLSTM(x)
        h_n = h_n.view(batch_size, output.size()[-1])  # 准备输入给Attention
        context_vector, attention_weights = self.Attention(h_n, output)  # 注意力 (batch_size, num_task, unit)
        ...
        return torch.stack(outs, dim=1)  # 返回一个张量列表
